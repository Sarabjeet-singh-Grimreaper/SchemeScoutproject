...
def find_matching_schemes(user_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for _, row in df.iterrows():
        ...
        result = {
            ...
            'application_deadline': str(scheme.get('Application Deadline', '')) if scheme.get('Application Deadline') else None,
            'application_url': scheme.get('Application URL', ''),
            'income_limit': scheme.get('Income Limit'),
            'min_age': scheme.get('Min Age'),
            'max_age': scheme.get('Max Age'),
